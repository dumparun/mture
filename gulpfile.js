...
gulp.task('buildandroid', function() {
	...
	runSequence('clean', 'copyLibs', 'uglify', 'copyAllFiles', 'cordovaAndroid');
});

gulp.task('fullbuildandroid', function() {

	runSequence('clean', 'copyLibs', 'uglify', 'copyAllFiles', 'cordovaAddAndroid',
	        'cordovaAndroid');
});
...
gulp.task('copyLibs', function() {
	...
	for ( var key in libsPath) {
		var destPath = libsPath[key].substring(0, libsPath[key].lastIndexOf("/"));
		gulp.src(paths.src + libsPath[key]).pipe(gulp.dest(paths.dest + '/' + destPath));
	}
});
